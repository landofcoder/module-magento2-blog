 ...
 */
namespace Ves\Blog\Block\Adminhtml\Feed;
...
class Edit extends \Magento\Backend\Block\Widget\Form\Container
{
    ...
    protected function _construct()
    {
        ...
        parent::_construct();
        if ($this->_isAllowedAction('Ves_Blog::feed_save')) {
        ...
        if ($this->_isAllowedAction('Ves_Blog::feed_delete')) {
    ...
    public function getHeaderText()
    {
        if ($this->_coreRegistry->registry('current_feed')->getId()) {
            return __("Edit Feed '%1'", $this->escapeHtml($this->_coreRegistry->registry('current_feed')->getTitle()));
        } else {
            return __('New Feed');
        }
    }
