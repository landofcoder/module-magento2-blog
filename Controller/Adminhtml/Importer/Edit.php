...
class Edit extends \Magento\Backend\App\Action
{
    ...
    {
        return $this->_authorization->isAllowed('Ves_Blog::importer');
    }
    ...
    public function execute()
    {
        ...
        $this->_coreRegistry->register('blog_importer', $model);

        // 5. Build edit form
        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->_initAction();
        $resultPage->addBreadcrumb(
            $id ? __('Edit Importer') : __('New Importer'),
            $id ? __('Edit Importer') : __('New Importer')
        );
        $resultPage->getConfig()->getTitle()->prepend(__('Importer'));
        $resultPage->getConfig()->getTitle()
            ->prepend($model->getId() ? $model->getTitle() : __('New Importer'));
